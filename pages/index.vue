<template>
    <div class="container mx-auto flex gap-x-6 mt-3">
        <!--sidbar-->
        <div
            class=" w-[384px] bg-white rounded-2xl p-4 shadow-[0_0px_32px_0px_rgba(125,109,114,0.12)]  h-[500px] overflow-y-auto">
            <p class="text-myPink text-base font-black mb-2">گفتگو ها</p>
            <chatCard />
            <chatCard />
            <chatCard />
        </div>
        <!--chat box-->
        <div class="flex-1 bg-white border rounded-2xl p-6 shadow-[0_0px_32px_0px_rgba(125,109,114,0.12)] h-[500px]">
            <div class="relative h-full">
                <!--userInfo-->
                <div class="flex gap-x-2 mb-6">
                    <div class="w-[48px] h-[48px] rounded-lg overflow-hidden">
                        <img
                            src="data:image/jpeg;base64,">
                    </div>
                    <div class="flex flex-col justify-between gap-y-1.5">
                        <span class="inline-block text-gray2 text-sm font-bold">{{ authUser.email }}</span>
                        <div class="text-gray3 text-xs font-medium">
                            <span class="">{{ digitsEnToFa("سطح 2 کاربری") }}</span>
                        </div>
                    </div>
                </div>

                <!--messages-->
                <div ref="msgBox" class="grid grid-cols-1 overflow-y-auto" id="messageBox">

                </div>

                <!--input message-->
                <div class="absolute bottom-0 mt-2 bg-grayLight rounded-lg p-3 flex w-full gap-x-3">
                    <div class="w-6 h-6 cursor-pointer" @click="sendMessage">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none">
                            <path fill="#FD2F70" fill-rule="evenodd"
                                d="M23.931 1.7c.336-1.009-.623-1.968-1.63-1.632L.88 7.208c-1.202.4-1.165 2.115.054 2.463l10.417 2.977 2.977 10.417c.348 1.219 2.062 1.256 2.463.053L23.932 1.7Zm-1.679.96L15.57 22.71l-2.971-10.396 9.654-9.654Zm-.912-.912-9.654 9.654L1.29 8.432l20.05-6.684Z"
                                clip-rule="evenodd" />
                        </svg>
                    </div>
                    <div class="w-6 h-6">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none">
                            <path fill="#BDBDBD"
                                d="M22.985 8.74a1 1 0 0 0-1.415.005L9.55 20.83a4 4 0 0 1-5.656-5.659L16.271 2.73a2.5 2.5 0 0 1 3.533 3.538L9.551 16.52l-.002.002a1 1 0 0 1-1.412-1.416l4.95-4.95a1 1 0 0 0-1.415-1.414l-4.949 4.95a3 3 0 1 0 4.243 4.242l.003-.004 10.25-10.25a4.5 4.5 0 0 0-6.364-6.364L2.478 13.76a6 6 0 0 0 8.487 8.483l12.023-12.088a1 1 0 0 0-.003-1.414Z" />
                        </svg>
                    </div>
                    <input type="text" v-model="msg"
                        class="w-full text-xs text-gray1 focus:ring-0 focus:outline-none bg-transparent">
                </div>
            </div>
        </div>

    </div>
</template>

<script setup>
import { digitsEnToFa } from "@persian-tools/persian-tools";
definePageMeta({
    middleware: 'auth',
})
const { authUser } = useAuth();
const msg = ref('');
const msgBox = ref(null);
const { $socket } = useNuxtApp();
let now;

onMounted(() => {
    $socket.io.on("broadcast", (msg) => {
        now = formatAMPM(new Date());
        const grayMsg = `<div class="row-span-1 flex justify-end mb-1">
        <div class="bg-gray6 max-w-[40%] rounded-lg rounded-bl-none px-4 pt-3 pb-1">
            <div class=" text-xs text-gray1 !leading-5 text-right mb-1">
                ${msg}
            </div>
            <div class="text-gray3 text-[10px]">${now}</div>
        </div>
    </div> `;
        msgBox.value.innerHTML += grayMsg;
    })
})

function sendMessage() {
    $socket.io.emit("messageEvent", msg.value);
    now = formatAMPM(new Date());
    const redMsg = `<div class="row-span-1 flex justify-start mb-1">
        <div class="bg-myPink max-w-[40%] bg-opacity-10 rounded-lg rounded-br-none px-4 pt-3 pb-1">
            <div class=" text-xs text-gray1 !leading-5 text-right mb-1">
               ${msg.value}
            </div>
            <div class="text-gray3 text-[10px]">${now}</div >
        </div >
    </div > `;
    msgBox.value.innerHTML += redMsg;
    msg.value = '';
}


</script>